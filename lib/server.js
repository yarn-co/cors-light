...
CorsLight.Server = (function() {
  ...
  Container.api = {
    ...
    set: function(originHostname, requestObj) {
      ...
      // Save
      this._storage.setItem(this.namespacedKey(key), JSON.stringify(store));
    ...
    get: function(originHostname, requestObj) {
      ...
      var loaded = this._storage.getItem(this.namespacedKey(key));
      ...
      if (!store) {
        ...
        if (store.expire !== false) {
          ...
          if (dateCheck < new Date()) {
            isExpired = true;
            this._storage.removeItem(this.namespacedKey(key)); // Delete expired keys
          }
        }
        ...
        if (store.session && store.session !== currentSession) {
          isExpired = true;
          this._storage.removeItem(this.namespacedKey(key));
        }
  ...
  Container.prototype.namespacedKey = function(key) {
